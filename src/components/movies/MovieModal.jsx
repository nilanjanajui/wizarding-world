      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl bg-background-dark border border-primary/20 flex flex-col">
        ...
        <div className="relative h-64 md:h-96 w-full flex-shrink-0">
          ...
          <div
            ...
            style={{ backgroundImage: `url("${supp.image}")` }}
          />
        ...
        <div className="p-6 md:p-10 space-y-8">
            ...
            <div className="flex gap-6 text-sm text-primary/80">
              {[
                ...
                <div key={label} className="flex flex-col">
                  <span className="uppercase text-[10px] tracking-widest opacity-60">{label}</span>
                  <span className="text-slate-100 font-medium">{value || "—"}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-3 w-full sm:w-auto">
              <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-background-dark px-6 py-3 rounded-lg font-bold transition-all hover:scale-[1.02]">
          ...
          <div className="bg-primary/5 rounded-xl p-6 border border-primary/10">
            <div className="flex items-center justify-between mb-4">
              <span className="text-lg font-bold text-slate-100">Wizard Audience Score</span>
              <div className="flex items-center gap-1 text-primary">
                {[...Array(4)].map((_, i) => (
                  <span key={i} className="material-symbols-outlined filled-icon">star</span>
                ))}
        ...
        {/* Accent footer */}
        <div className="h-1 bg-gradient-to-r from-gryffindor-red via-primary to-gryffindor-red opacity-60" />
      </div>
    </div>
  );
}
